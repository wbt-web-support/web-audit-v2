-- Safe setup script for admin alerts table
...
DROP TRIGGER IF EXISTS trigger_update_admin_alerts_updated_at ON admin_alerts;

-- Drop existing function if it exists
DROP FUNCTION IF EXISTS auto_expire_admin_alerts();
...
CREATE INDEX idx_admin_alerts_created_by ON admin_alerts(created_by);
CREATE INDEX idx_admin_alerts_start_date ON admin_alerts(start_date);
CREATE INDEX idx_admin_alerts_end_date ON admin_alerts(end_date);
...
-- Create function to auto-expire alerts
...
-- Insert sample data only if an admin user exists
DO $$
DECLARE
    admin_user_id UUID;
BEGIN
