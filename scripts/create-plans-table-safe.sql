...
) AS v(name, description, plan_type, amount, currency, interval_type, features, limits, is_popular, color, sort_order)
WHERE NOT EXISTS (SELECT 1 FROM plans WHERE plan_type = v.plan_type);
