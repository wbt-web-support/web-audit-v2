...
function determineKeyStatus(key: string, context: string): 'exposed' | 'secure' | 'warning' {
  ...
  if (lowerContext.includes('environment') || lowerContext.includes('process.env') || lowerContext.includes('config') || lowerContext.includes('secret') || lowerContext.includes('private')) {
    return 'secure';
  }

  // Check for exposed patterns
  if (lowerContext.includes('console.log') || lowerContext.includes('alert') || lowerContext.includes('document.write') || lowerContext.includes('innerhtml')) {
    return 'exposed';
  }
...
export async function detectKeysInHtml(htmlContent: string): Promise<KeyDetectionResult> {
  ...
  const uniqueKeys = detectedKeys.filter((key, index, self) => index === self.findIndex(k => k.key === key.key && k.type === key.type));
...
export async function detectKeysInPages(pages: Array<{
  pageName: string;
  pageUrl: string;
  pageHtml: string;
}>): Promise<{
  ...
  const uniqueKeys = allKeys.filter((key, index, self) => index === self.findIndex(k => k.key === key.key && k.type === key.type));
