  ...
  const renderTabContent = () => {
    ...
    return <div className="space-y-3">
        ...
        <div className="space-y-3">
          {issues.map((issue, index) => <div key={index} className={`p-4 rounded-lg border-l-4 ${issue.severity === 'high' ? 'bg-red-50 border-red-400' : issue.severity === 'medium' ? 'bg-yellow-50 border-yellow-400' : 'bg-[#ff4b01]/10 border-[#ff4b01]/50'}`}>
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${issue.severity === 'high' ? 'bg-red-100 text-red-800' : issue.severity === 'medium' ? 'bg-yellow-100 text-yellow-800' : 'bg-[#ff4b01]/20 text-[#ff4b01]'}`}>
  ...
  return <div className="space-y-6">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        <div className="flex items-center justify-between mb-6">
          ...
          {!geminiAnalysis && !isStreaming && <button onClick={handleReAnalyze} disabled={isStreaming} className="px-4 py-2 bg-[#ff4b01] text-white rounded-lg hover:bg-[#e64401] disabled:bg-gray-400 disabled:cursor-not-allowed transition-colors flex items-center gap-2">
        ...
        {isStreaming ? <div className="text-center py-12">
            ...
            }].map(tab => <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)} className={`whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm flex items-center gap-2 ${activeTab === tab.id ? 'border-[#ff4b01] text-[#ff4b01] bg-[#ff4b01]/10' : 'border-transparent text-gray-500 hover:text-[#ff4b01] hover:border-[#ff4b01]/30'}`}>
                    {tab.label}
                    <span className={`px-2 py-1 rounded-full text-xs ${activeTab === tab.id ? 'bg-[#ff4b01]/20 text-[#ff4b01]' : 'bg-gray-100 text-gray-600'}`}>
                      {tab.count}
                    </span>
                  </button>)}
              </nav>
