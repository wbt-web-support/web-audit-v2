  ...
  const loadPlans = useCallback(async () => {
    ...
    try {
      const {
        data: directData,
        error: directError
      } = await supabase.from('plans').select('id, name, description, plan_type, can_use_features, max_projects, is_active, created_at, updated_at').order('created_at', {
        ascending: true
      });
  ...
  const handleSaveFeatures = async () => {
    ...
    try {
      ...
      const {
        data: checkData,
        error: checkError
      } = await supabase.from('plans').select('id, name, can_use_features').eq('id', selectedPlan.id).single();
      if (checkError) {
      ...
      }).eq('id', selectedPlan.id).select().single();
      if (error) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        delay: 0.2
          ...
          {plansLoading ? <div className="flex items-center justify-center py-8">
              ...
              {filteredPlans.map((plan, index) => <motion.div key={plan.id} initial={{
            opacity: 0,
            y: 20
          }} animate={{
            opacity: 1,
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      <div className="flex items-center space-x-3">
                        ...
                        {!plan.is_active && <span className="inline-flex px-2 py-1 text-xs font-semibold rounded-full bg-red-100 text-red-800">
                            Inactive
                          </span>}
                      </div>
        ...
        delay: 0.3
          ...
          {selectedPlan ? <div className="p-6 max-h-96 overflow-y-auto">
              ...
              <div className="flex gap-2 mb-6">
                ...
                <button onClick={handleClearFeatures} className="px-3 py-1 bg-gray-100 text-gray-700 rounded text-sm hover:bg-gray-200 transition-colors">
                  Clear All
                </button>
              </div>
                      ...
                      {categoryFeatures.map(feature => <label key={feature.id} className="flex items-start space-x-3 p-3 rounded-lg border border-gray-200 hover:border-blue-300 hover:bg-blue-50 transition-colors cursor-pointer">
                          ...
                          <div className="flex-1">
                            <div className="flex items-center space-x-2">
                              ...
                              {feature.isCore && <span className="px-2 py-1 text-xs bg-green-100 text-green-700 rounded-full">
                                  Core
                                </span>}
                            </div>
      ...
      delay: 0.4
          ...
          <div className="text-sm text-gray-600">
            ...
            return feature ? <span key={featureId} className="inline-flex items-center space-x-1 px-2 py-1 bg-blue-100 text-blue-700 rounded text-xs">
              ...
              </div> : <span className="text-gray-500">No features selected</span>}
          </div>
        </motion.div>}
    </motion.div>;
}