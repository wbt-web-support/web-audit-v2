...
export default function AdminPlans({
  ...
  const [editingCreditPackage, setEditingCreditPackage] = useState<any | null>(null);
  ...
  // Load plans and credit packages on component mount
  useEffect(() => {
    loadPlans();
    loadCreditPackages();
  }, [loadPlans, loadCreditPackages]);
  ...
  const handleCreditPackageAction = async (packageId: string | null, action: 'create' | 'update' | 'delete', data?: any) => {
    try {
      let response;
      if (action === 'create') {
        response = await fetch('/api/credit-packages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data)
        });
      } else if (action === 'update') {
        response = await fetch(`/api/credit-packages/${packageId}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(data)
        });
      ...
      if (response && response.ok) {
        ...
        alert(`Credit package ${action}d successfully!`);
        ...
        alert(errorData.message || `Failed to ${action} credit package`);
      }
    } catch (error) {
      console.error(`Error ${action}ing credit package:`, error);
      alert(`Failed to ${action} credit package`);
    }
  };

  const handleEditCreditPackage = (pkg: any) => {
                ...
                {creditPackages.length === 0 ? (
                  ...
                  creditPackages.map((pkg, index) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-black">₹{Number(pkg.price).toLocaleString()}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreditPackageForm({ ...creditPackageForm, credits: e.target.value })}
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreditPackageForm({ ...creditPackageForm, price: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="100.00"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreditPackageForm({ ...creditPackageForm, label: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setCreditPackageForm({ ...creditPackageForm, sort_order: parseInt(e.target.value) || 0 })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  placeholder="0"
                />
              </div>
              ...
              {creditPackageForm.credits && creditPackageForm.price && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3">
                  <p className="text-sm text-blue-800">
                    <strong>Price per Credit:</strong> ₹{(parseFloat(creditPackageForm.price) / parseInt(creditPackageForm.credits)).toFixed(2)}
                  </p>
                </div>
              )}
            </div>
