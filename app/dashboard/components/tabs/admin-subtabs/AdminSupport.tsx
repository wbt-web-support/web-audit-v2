...
export default function AdminSupport({ userProfile }: AdminSupportProps) {
  ...
  const handleUpdateTicketStatus = async (ticketId: string, newStatus: string) => {
    try {
      ...
      console.log('Update ticket status response:', { data, error, hasData: !!data, hasError: !!error })
  ...
  const supportStats = {
    ...
    averageResolutionTime: '4.2 hours', // This would need to be calculated from actual data
    customerSatisfaction: '4.8/5', // This would need to be calculated from actual data
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          ...
          { label: 'In Progress', value: supportStats.inProgressTickets, color: 'yellow' },
          { label: 'Resolved', value: supportStats.resolvedTickets, color: 'green' },
          { label: 'Closed', value: supportStats.closedTickets, color: 'gray' },
          { label: 'High Priority', value: supportStats.highPriorityTickets, color: 'orange' }
        ].map((stat, index) => (
          <motion.div
            key={stat.label}
            className="bg-white rounded-lg border border-gray-200 p-6"
