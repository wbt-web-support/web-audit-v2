...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: allPayments, error: allPaymentsError } = await supabaseServiceClient
      .from('payments')
      .select('id, user_id, plan_name, payment_status')
      .limit(5);

    console.log('All payments in database (first 5):', {
    ...
    const { data: payments, error: paymentsError } = await supabaseServiceClient
      .from('payments')
      .select(`
        id,
        razorpay_payment_id,
