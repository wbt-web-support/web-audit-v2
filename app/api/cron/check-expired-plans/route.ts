...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: usersWithExpiredPlans, error: fetchError } = await supabaseServiceClient
      .from('users')
      .select('id, email, plan_type, plan_id, plan_expires_at, billing_cycle')
    ...
    console.log(`Found ${usersWithExpiredPlans?.length || 0} users with expired plans`);
    ...
    for (const user of usersWithExpiredPlans) {
      try {
        console.log(`Processing expired plan for user: ${user.id} (${user.email})`);
        ...
        console.log(`Successfully updated user ${user.id} to Starter plan`);
    ...
    console.log(`Processed ${processedUsers.length} users, ${errors.length} errors`);
