...
export async function POST(request: NextRequest) {
  try {
    console.log('=== PLAN EXPIRY CHECK API CALLED ===');
    ...
    console.log('Checking plan expiry for user:', user.id);

    // Get user's current plan data
    const { data: userData, error: userDataError } = await supabaseServiceClient
      .from('users')
      .select('id, plan_type, plan_id, plan_expires_at, billing_cycle')
      .eq('id', user.id)
      .single();
    ...
    console.log('Plan has expired, downgrading to Starter plan...');

    // Get Starter plan details
    const { data: starterPlan, error: starterPlanError } = await supabaseServiceClient
      .from('plans')
    ...
    const { data: updateResult, error: updateError } = await supabaseServiceClient
      ...
      .update({
        ...
        plan_expires_at: null, // Starter plan doesn't expire
        billing_cycle: null,
        updated_at: new Date().toISOString()
      })
      .eq('id', user.id)
      .select('id, plan_type, plan_id, max_projects, can_use_features');
    ...
    console.log('User plan updated to Starter:', updateResult);
...
export async function GET(request: NextRequest) {
  try {
    console.log('=== PLAN EXPIRY STATUS CHECK API CALLED ===');
    ...
    console.log('Checking plan expiry status for user:', user.id);
    ...
    console.log('Plan expiry status:', {
      now: now.toISOString(),
      expiryDate: expiryDate?.toISOString(),
      isExpired,
      daysUntilExpiry
